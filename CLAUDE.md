# Claude Code Project Instructions — rama-tls-rustls-kotlin

## Project Overview

This is **rama-tls-rustls-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`rama-tls-rustls`](https://crates.io/crates/rama-tls-rustls).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/rama-tls-rustls/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/rama-tls-rustls>
